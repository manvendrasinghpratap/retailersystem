                ...
                <div class="card-body">
                        <form method="POST" action="{{ $route == 'add' ? route('administrator.account.store') : route('administrator.account.update') }}" enctype="multipart/form-data" class="needs-validation" novalidate onSubmit="return validatedata();">
                            ...
                            <div class="row">
                                ...
                                <x-select-dropdown name="local_government" label="Local Government" :options="$localGovernment" :selected="$accountdetails->user->detail->local_government ?? 'Aba North'" required class="local_government"/>
