            ...
            <div class="card-body">
                    <form method="POST" action="{{ route(array_key_exists('route',$breadcrumb)?$breadcrumb['route']:'javascript::void(0)') }}" enctype="multipart/form-data" class="needs-validation" novalidate autocomplete="false" onSubmit="return validate();">
                        ...
                        <div class="row">                            
                            ...
                            <x-text-input name="password" label="New Password" value="" id="password" required class="password form-control" mainrows="4"/>
...
<script>
    ...
    function validate() {
        ...
        if (newpassword.toLocaleLowerCase().indexOf("password") != -1) {
            $('.datavalidationerror').html('New password should not contain the password keyword.');
            valid =false;
        }
        alert(valid)
        return valid;
