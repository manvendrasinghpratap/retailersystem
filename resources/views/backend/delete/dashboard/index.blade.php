        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-inline-block">
                        ...
                        'pdfId' =>'dispatchedanddelivered',    
                        'pdfRoute' => route('dispatchedanddelivered'),
                        'pdfClass' => 'dispatchedanddelivered',
                        'csvId' =>'dispatchedanddeliveredcsv',    
                        'csvRoute' => route('dispatchedanddeliveredcsv'),
                        'csvClass' => 'dispatchedanddeliveredcsv',
                        ])                 
                    </div>
            </div>
            <div class="card-body">
                {{-- Filter Form --}}
                     <form method="GET" action="{{ route(array_key_exists('route',$breadcrumb) ? $breadcrumb['route'] : '') }}" class="mb-3"> 
                        <div class="row">
                            ...
                            <x-select-dropdown name="delivery_staff_id" label="{{ __('translation.delivery_staff') }}" :options="$staffList" :selected="request('delivery_staff_id')" class="staff" mainrows="2"/>
                            <x-select-dropdown name="status" label="{{ __('translation.status') }}" :options="\Config::get( 'constants.deliver_order_status' )" :selected="request('status')" class="accountstatus" mainrows="2"/>
                            <x-date-input name="from_date" :label="__('translation.from_date')" data-mindate="{{\App\Helpers\Settings::getFormattedDate(date('Y-m-d', strtotime('-20 years')) )}}" data-maxdate="{{\App\Helpers\Settings::getFormattedDate(date('Y-m-d', strtotime('0 days')) )}}" value="{{ request('from_date') ?? \App\Helpers\Settings::getFormattedDate(date('Y-m-d')) }}" class="flatdatepickr " mainrows="2"/>
                            <x-date-input name="to_date" :label="__('translation.to_date')" data-mindate="{{\App\Helpers\Settings::getFormattedDate(date('Y-m-d', strtotime('-20 years')) )}}" data-maxdate="{{\App\Helpers\Settings::getFormattedDate(date('Y-m-d', strtotime('0 days')) )}}" value="{{ request('to_date') ?? \App\Helpers\Settings::getFormattedDate(date('Y-m-d')) }}" class="flatdatepickrto" mainrows="2"/>
                        ...
                        <tbody>
                            @forelse($orders as $order)
                                <tr>
                                    ...
                                    <td><a @if($order->status == 'delivered') title="{{ __('translation.deliver_to') . ' : ' . ($order->orderdelivery->deliver_to ?? '') . ' | ' . __('translation.notes') . ' : ' . ($order->orderdelivery->note ?? '') . ' | ' . __('translation.delivered_date') . ' : ' . ($order->orderdelivery->delivered_date ??  '') }}" @else title = "" @endif href="javascript:void(0);" class="order_comments- @if($order->status == 'delivered') success @endif ">{{ $order->customer->customer_name }}</a></td>
                                    <td class="productdiv">
                                    ...
                                    <td @if($order->status == 'delivered' ) title="{{ __('translation.deliver_to') . ' : ' . ($order->orderdelivery->deliver_to ?? '') . ' | ' . __('translation.notes') . ' : ' . ($order->orderdelivery->note ?? '') . ' | ' . __('translation.delivered_date') . ' : ' . ($order->orderdelivery->delivered_date ?? '') }}" 
                                    ...
                                    <td>
                                        ...
                                        {{-- <x-href-input target='_blank' name="printOrder" href="{{ route('order.printinvoice',['id' => \App\Helpers\Settings::getEncodeCode($order->id)]) }}" action="print" /> --}}
                                    </td>
                                </tr>
                            @empty
                                <tr>
...
<script>
    $(document).ready(function() {
       setupPdfDownload('.dispatchedanddelivered', 'data-downloadroutepdf');
    });
</script>
@endsection
