                ...
                <div class="card-body">
                    <form name="cartlistingform" id="cartlistingform" method="GET">
                        <div class="row">
                            ...
                            <x-text-input name="from_date" :label="__('translation.from_date')" :value="request('from_date')" mainrows="2"
                                class="flatdatepickr" />
                            <x-text-input name="to_date" :label="__('translation.to_date')" :value="request('to_date')" mainrows="2"
                                class="flatdatepickr" />
                            <x-select-dropdown name="payment_type" label="Payment Type" :options="$paymentTypes" :selected="request('payment_type')"
                                class="payment_type" mainrows="2" />
                            <x-select-dropdown name="approval_status" :label="__('translation.approval_status')" :options="\Config::get('constants.approvalStatus')" :selected="request('approval_status')"
                                class="approval_status" mainrows="2" />
                            ...
                            <tbody>
                                @forelse($sales as $sale)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="javascript:void(0)" class="payment-status-btn"
                                                data-heading-title="{{ ucfirst($paymentTypes[$sale->payment_type] ?? '-') }} Details"
                                            ...
                                            <div class="d-inline-flex align-items-center justify-content-center gap-2">
                                                ...
                                                @if (empty($sale->payment_approved_by) && $sale->payment_approval_status === 'pending')
                                                    <a href="javascript:void(0)"
                                                        class="btn btn-sm btn-soft-warning approve-credit-sale-btn px-2 py-1"
                                                        data-customer-name="{{ $sale->customer->name ?? 'Guest' }}"
                                                        data-customer-contact="{{ $sale->customer->phone ?? ($sale->customer->email ?? '-') }}"
                                                        data-total-amount="{{ __('translation.b_ngn') }}{{ \App\Helpers\Settings::getcustomnumberformat($sale->total) }}"
                                                        data-sale-id="{{ \App\Helpers\Settings::getEncodeCodeWithHashids($sale->id) }}"
                                                        data-invoice-no="{{ $sale->invoice_no }}" data-bs-toggle="tooltip"
                                                        data-bs-placement="top" title="Approve / Reject Credit Sale">
                                                        <i class="fas fa-file-signature text-warning"></i>
                                                    </a>
                                                @endif

                                                {{-- View Details Action --}}
                                                <a href="{{ route('admin.sales.show', \App\Helpers\Settings::getEncodeCodeWithHashids($sale->id)) }}"
                                                    class="btn btn-sm btn-soft-primary- px-2 py-1" data-bs-toggle="tooltip"
                                                ...
                                                @if (!empty($sale->payment_approved_by) && $sale->payment_approval_status === 'approve')
                                                    <a href="{{ route('downloadinvoice', \App\Helpers\Settings::getEncodeCodeWithHashids($sale->id)) }}"
                                                        class="btn btn-sm btn-soft-success- px-2 py-1" target="_blank"
                                                    ...
                                                    <a href="javascript:void(0)"
                                                        class="btn btn-sm btn-soft-info- payment-status-btn px-2 py-1"
                                                        data-heading-title="{{ ucfirst($paymentTypes[$sale->payment_type] ?? '-') }} Details"
                                                        data-sale-id="{{ $sale->id }}" data-bs-toggle="tooltip"
        ...
        $(document).on('click', '.payment-status-btn', function() {
            ...
            $.get(url, function(res) {
                ...
                if (sale.payments && sale.payments.length > 0) {
                    ...
                    history +=
                        `<tr><td colspan="4" class="text-center">No payment history available</td></tr>`;
                }
