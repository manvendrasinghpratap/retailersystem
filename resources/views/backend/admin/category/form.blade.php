    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                         <form method="POST" id="productcatform" name="productcatform" action="{{ isset($category) ? route('admin.categories.update', \App\Helpers\Settings::getEncodeCode($category->id)) : route('admin.categories.store') }}" enctype="multipart/form-data" class="needs-validation" novalidate >
                            @csrf
                            ...
                            <div class="row">
                                ...
                                <x-text-input name="slug" label="Slug" value="{{ $category->slug ?? '' }}" required class=""/>
                                <x-select-dropdown name="status" label="Status" :options="config('constants.accountstatus')" :selected="$category->status ?? 1" />
                                <x-image-upload name="image" label="Image" :value="$category->image ?? ''" />
                                <x-textarea-input name="description" label="Short Description" value="{{ $category->description ?? '' }}"  class="" mainrows="4"/>
                            </div>
                             <div class="row">
