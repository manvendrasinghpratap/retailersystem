                    ...
                    <form name="addcustomerform" id="addnewstockform" method="POST" action="{{ route('admin.stock.adjust') }}" enctype="multipart/form-data" class="needs-validation" novalidate autocomplete="off">
                        ...
                        <div class="row">
                            ...
                            @if(in_array($adjustment ?? '', ['damage', 'deduct', 'expired']))
                                ...
                                ]" :selected="old('reason')" required class="form-control required-select" mainrows="4" />
                            ...
                            {{-- Note --}}
                            <x-textarea-input name="note" :label="in_array($adjustment ?? '', ['damage', 'deduct'])
            ? __('translation.reason_note')
            : __('translation.note')" :value="$masterItemName ?? ''" class="" :placeholder="__('translation.please_enter_note')" :required="!empty($adjustment ?? false)" :mainrows="12" rows="2" />
