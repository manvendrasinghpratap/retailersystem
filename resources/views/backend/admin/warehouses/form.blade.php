                ...
                <div class="card-body">
                    <form autocomplete="off" method="POST" id="warehouseform" name="warehouseform" action="{{ isset($warehouse) ? route('admin.warehouses.update', \App\Helpers\Settings::getEncodeCode($warehouse->id)) : route('admin.warehouses.store') }}" class="needs-validation" novalidate>
                        ...
                        <div class="row">
                            ...
                            {{-- Phone --}}
                            <x-text-input name="phone" label="{{ __('translation.phone') }}" value="{{ $warehouse->phone ?? '' }}" required class="onlyinteger" maxlength="12" minlength="11" />
                            {{-- Email --}}
                            ...
                            {{-- Status --}}
                            <x-select-dropdown name="status" label="{{ __('translation.status') }}" :options="config('constants.accountstatus')" :selected="isset($warehouse) && $warehouse->status == 0 ? 0 : 1" required class="accountstatus" />

                        </div>

                        {{-- Buttons --}}
                        <div class="row">
                            <x-form-buttons submitText="{{ isset($warehouse) ? 'Update' : 'Save' }}" resetText="{{ $breadcrumb['reset_route_title'] }}" url="{{ route($breadcrumb['reset_route']) }}" />
                        </div>
