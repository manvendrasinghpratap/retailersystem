                ...
                <div class="card-body">
                    ...
                    <form
                        ...
                        <div class="row mb-3 payment-methods">
                            <x-text-input name="payment_cash" id="payment_cash" :label="__('translation.cash')" :value="(isset($order) && $order->orderpaymentdetails()->count() > 0) ? $order->orderpaymentdetails[0]->payment_cash : ''" required class="payment-amount default-zero onlyinteger" type="number" min="0" :disabled="!empty($order) && $order->orderpaymentdetails()->count() > 0" />
                            <x-text-input name="payment_pos" id="payment_pos" :label="__('translation.pos')" :value="(isset($order) && $order->orderpaymentdetails()->count() > 0) ? $order->orderpaymentdetails[0]->payment_pos : ''" required class="payment-amount default-zero onlyinteger" type="number" min="0" :disabled="!empty($order) && $order->orderpaymentdetails()->count() > 0" />
                           <x-text-input name="payment_transfer" id="payment_transfer" :label="__('translation.transfer')" :value="(isset($order) && $order->orderpaymentdetails()->count() > 0) ? $order->orderpaymentdetails[0]->payment_transfer : ''" required class="payment-amount default-zero onlyinteger" type="number" min="0" :disabled="!empty($order) && $order->orderpaymentdetails()->count() > 0" />
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#orderForm').on('submit', function(e) {
                ...
                var paymentCash = parseFloat($('#payment_cash').val()) || 0;
                var paymentPos = parseFloat($('#payment_pos').val()) || 0;
                var paymentTransfer = parseFloat($('#payment_transfer').val()) || 0;
                var totalPayment = paymentCash + paymentPos + paymentTransfer;

                // ✅ Get order total (from your total field)
                var totalOrderAmount = parseFloat($('#total_amount').val()) || 0;

                // ✅ Validate payment values
                if (paymentWay && totalPayment <= 0) {
                    isValid = false;
                    $('#payment_error').text("Please enter at least one payment amount.");
                    $('#payment_cash, #payment_pos, #payment_transfer').addClass('is-invalid');
                } 
                else if (paymentWay && totalPayment !== totalOrderAmount) {
                    isValid = false;
                    $('#payment_error').text(
                        "Total payment (" + totalPayment.toFixed(2) + 
                        ") must equal order amount (" + totalOrderAmount.toFixed(2) + ")."
                    );
                    $('#payment_cash, #payment_pos, #payment_transfer').addClass('is-invalid');
                } 
                else {
                    $('#payment_error').text('');
                    $('#payment_cash, #payment_pos, #payment_transfer').removeClass('is-invalid');
                }
                ...
                if (!isValid) {
                    e.preventDefault();
                    $('html, body').animate({
                        scrollTop: $("#payment_way").offset().top - 100
                    }, 500);
                }
            });
